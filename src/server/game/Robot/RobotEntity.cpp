...
void RobotEntity::Update(uint32 pmDiff)
{
    ...
    if (checkDelay < 0)
    {
        ...
        case RobotEntityState::RobotEntityState_Enter:
        {
            ...
            sLog->outMessage("lfm", LogLevel::LOG_LEVEL_INFO, "Robot %s is ready to go online.", account_name);
            break;
        }
        case RobotEntityState::RobotEntityState_CheckAccount:
        {
            ...
            else
            {
                ...
                if (account_id > 0)
                {
                    sLog->outMessage("lfm", LogLevel::LOG_LEVEL_INFO, "Robot %s is ready.", account_name);
                    entityState = RobotEntityState::RobotEntityState_CheckCharacter;
                }
                else
                {
                    sLog->outMessage("lfm", LogLevel::LOG_LEVEL_INFO, "Robot %s is not ready.", account_name);
        ...
        case RobotEntityState::RobotEntityState_CreateAccount:
        {
            ...
            else
            {
                ...
                {
                    sLog->outMessage("lfm", LogLevel::LOG_LEVEL_INFO, "Robot id %d create account failed.", robot_id);
                    entityState = RobotEntityState::RobotEntityState_None;
                }
                else
                {
